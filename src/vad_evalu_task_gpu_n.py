...
from utils.decode_utils import predict_gpu, smooth, get_full_conf, get_full_conf2
...
for index,wav_key in enumerate(tqdm(test_utt2wav.keys())):
    ...
    if ".wav" in wav_path:
        ...
        raw_feats = torch_fb(torch.from_numpy(all_signal).float())
        ...
        if len(feats) <= word_num:
            print(f"{wav_key} {len(feats)}")
            continue
        out = predict_gpu(net, feats)
        ...
        # if len(signal) < 1.5 * 16000:
        #     signal = np.pad(signal, (0,int(1.5 * 16000 - len(signal))), mode="wrap")
        # for seg in segments:

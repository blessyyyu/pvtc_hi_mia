...
data_train_root=${kws_corpus_dir}/train
data_dev_root=${kws_corpus_dir}/dev
data_test_root=${kws_corpus_dir}/test
...
if [ $stage -le 0 ];then
    ...
    if [ -f $data_train_root/SPEECHDATA/train.scp ];then
        ...
        awk -v kws_corpus_dir=${kws_corpus_dir} '{print kws_corpus_dir"/train/SPEECHDATA/"$0 }' $data_train_root/SPEECHDATA/train.scp | grep -E '_02_|_10_' > $data_train_root/SPEECHDATA/train_p.scp
		# mv $data_train_root/SPEECHDATA/train_p.scp $data_train_root/wav.scp
        cp $data_train_root/SPEECHDATA/train_p.scp $data_train/wav.scp && rm $data_train_root/SPEECHDATA/train_p.scp
    fi
    if [ -f $data_dev_root/SPEECHDATA/dev.scp ];then
        ...
        awk -v kws_corpus_dir=${kws_corpus_dir} '{print kws_corpus_dir"/dev/SPEECHDATA/"$0 }' $data_dev_root/SPEECHDATA/dev.scp | grep -E '_02_|_10_'  > $data_dev_root/SPEECHDATA/dev_p.scp
        # mv $data_dev_root/SPEECHDATA/dev_p.scp $data_dev_root/wav.scp
        cp $data_dev_root/SPEECHDATA/dev_p.scp $data_dev/wav.scp && rm $data_dev_root/SPEECHDATA/dev_p.scp
	fi
    # awk '{print "data/kws/test/wav/"$0}' $data_test_root/wav.scp  > $data_test_root/test_p.scp
    awk -v kws_corpus_dir=${kws_corpus_dir} '{print kws_corpus_dir"/test/wav/"$0 }' $data_test_root/wav.scp  > $data_test_root/test_p.scp
	# mv $data_test_root/test_p.scp $data_test/wav.scp
    cp $data_test_root/test_p.scp $data_test/wav.scp && rm $data_test_root/test_p.scp
fi

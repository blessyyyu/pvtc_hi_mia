...
# BAC009S0002W0125.npy
# haomi_SV0001_2_02_F0672.npy
pos_spk = [item.split('_')[1] for item in open(root_dir + "/pos_list")]
neg_spk = [item[6:11] for item in open(root_dir + "/neg_list")]
print(f'pos_spk[0]: {pos_spk[0]}')
print(f'neg_spk[0]: {neg_spk[0]}')
pos_spk = list(set(pos_spk))
neg_spk = list(set(neg_spk))
...
# train_spk = spk[:290]
# dev_spk = spk[290:300]
pos_train_spk = pos_spk[:int(0.95 * len(pos_spk))]
pos_dev_spk = pos_spk[int(0.95 * len(pos_spk)):]
neg_train_spk = neg_spk[:int(0.95 * len(neg_spk))]
neg_dev_spk = neg_spk[int(0.95 * len(neg_spk)):]

# print("pos_list speaker number:",len(spk))
# print("train speaker number:",len(train_spk))
# print("dev speaker number:",len(dev_spk))

print("pos_list speaker number:",len(pos_spk))
print("pos train speaker number:",len(pos_train_spk))
print("pos dev speaker number:",len(pos_dev_spk))
print("neg_list speaker number:",len(neg_spk))
print("neg train speaker number:",len(neg_train_spk))
print("neg dev speaker number:",len(neg_dev_spk))
...
#     spk_id += 1
for spk_item in pos_train_spk:
    f_spk2id.writelines(spk_item + " " + str(spk_id) + "\n")
    spk_id += 1
for spk_item in neg_train_spk:
    f_spk2id.writelines(spk_item + " " + str(spk_id) + "\n")
    spk_id += 1
f_spk2id.close()
...
#     spk_id += 1
for spk_item in pos_dev_spk:
    f_spk2id_test.writelines(spk_item + " " + str(spk_id) + "\n")
    spk_id += 1
for spk_item in neg_dev_spk:
    f_spk2id_test.writelines(spk_item + " " + str(spk_id) + "\n")
    spk_id += 1
f_spk2id_test.close()
...
for item in open(root_dir + "/pos_list"):
    item = item.strip()
    # spk_item = item.split('-')[0].split("_")[-1]
    spk_item = item.split('_')[1]
    if spk_item in pos_spk:
        if spk_item not in data_dict.keys():
            data_dict[spk_item] = []
        data_dict[spk_item].append(item)
...
for spk in tqdm(data_dict.keys()):
    ...
    for index,utt in enumerate(utt_list):
        # if spk in train_spk:
        if spk in pos_train_spk:
            train_utt2wav.append(utt + ' ' + pos_dir + utt +  '\n')
        # elif spk in dev_spk:
        elif spk in pos_dev_spk:
...
for spk in tqdm(neg_data_dict.keys()):
    ...
    for index,utt in enumerate(utt_list):
        # if spk in train_spk:
        if spk in neg_train_spk:
            train_utt2wav.append(utt + ' ' + neg_dir + utt +  '\n')
            i += 1
        # elif spk in dev_spk:
        elif spk in neg_dev_spk:
